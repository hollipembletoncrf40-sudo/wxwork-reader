...
def main():
    ...
    stencil_bu_dept = "嘉立创 / 激光钢网事业部"
    stencil_mkt_dept = "嘉立创 / 激光钢网事业部 / 钢网市场部"
    smt_bu_dept = "嘉立创 / SMT 事业部"
    # 3D 打印各分支
    d3_bu_dept = "嘉立创 / 3D 打印事业部"
    d3_mkt_dept = "嘉立创 / 3D 打印事业部 / 3D 市场部"
    d3_sg_dept = "嘉立创 / 3D 打印事业部 / 3D 市场部 / 韶关市场组"
    d3_it_dept = "嘉立创 / 3D 打印事业部 / 3D打印 IT分部"
    d3_cs_dept = "嘉立创 / 3D 打印事业部 / 3D客服部"

    # CNC 智造事业部
    cnc_bu_dept = "嘉立创 / CNC 智造事业部"
    cnc_it_innov_dept = "嘉立创 / CNC 智造事业部 / CNC IT分部 / 创新项目组"
    cnc_it_dev_dept = "嘉立创 / CNC 智造事业部 / CNC IT分部 / IT工厂开发组"
    cnc_it_prod_dept = "嘉立创 / CNC 智造事业部 / CNC IT分部 / IT工厂产品组"
    cnc_mkt_dept = "嘉立创 / CNC 智造事业部 / CNC市场部"

    # FPC 业务部
    fpc_bu_dept = "嘉立创 / FPC 柔性线路板事业部"

    # 研发与跨部门沟通组
    rnd_comm_dept = "嘉立创 / 技术研发中心 / SMT-EDA与软件协同组"
    ...
    dept_mappings = [
        ...
        # 激光钢网事业部
        (["刘金花"], stencil_bu_dept),
        (["罗天珍", "徐海洋", "尹亮", "周丽英", "李佳民", "郑顺彦"], stencil_mkt_dept),
        # SMT 事业部 (国内制造与事业部)
        (["贺定球"], smt_bu_dept),
        # 3D 打印事业部
        (["罗志文"], d3_bu_dept),
        (["陈锐", "全友彬", "欧耀欢"], d3_mkt_dept),
        (["潘锦波"], d3_sg_dept),
        (["陈皓培", "林恒宇", "刘添", "饶伟烽", "王昕", "罗思梅", "张威", "蔡诗茵", "宋万兵", "李月", "沈志华", "金智伟", "王海涛", "刘龙", "谢治邦", "黄同德"], d3_it_dept),
        (["陈映玲", "吉彬鑫", "朱雨晴", "周振兴", "童育龙", "罗丝雨", "于奇巧", "颜阳"], d3_cs_dept),
        # CNC 智造事业部
        (["许明灯"], cnc_bu_dept),
        (["何建深", "卢积龙", "麻红远", "蒙德贵", "黄志杰", "蒋艺", "李明", "牟函", "吴海瑞", "周海锋", "张铁山", "吴荣鑫", "李鹏博"], cnc_it_innov_dept),
        (["陈思丹", "高峰", "王招办", "吴水华", "肖智文", "谢定伟", "谢宏松", "俞开烜", "周君"], cnc_it_dev_dept),
        (["郝杰鹏", "何晓梅", "黄华章", "肖华为", "张祖铭"], cnc_it_prod_dept),
        (["欧海玉", "周雪雪"], cnc_mkt_dept),
        # FPC 柔性板事业部
        (["张澳宇", "李中萍", "陈宝瑜", "何琳", "钟李花", "魏乐", "苗旭楠", "林俏华", "彭红茂", "胡国梁", "幸谦"], fpc_bu_dept),
        # SMT-EDA 与 软件协同
        (["陈智峰", "郭煜", "黄桐鑫", "罗德松", "彭小斌", "谢宇森", "熊芝伟", "侯召军", "王凯", "周金明", "王家体", "陈梦奇"], rnd_comm_dept)
    ]
